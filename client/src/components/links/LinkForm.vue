...
<template>
  <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 shadow-sm">
    <div class="flex items-center justify-between mb-4">
      ...
      <button @click="cancel" class="text-gray-400 hover:text-gray-600"><XMarkIcon class="w-5 h-5" /></button>
    </div>

    <form @submit.prevent="handleSubmit" class="space-y-3">
      <input v-model="url" type="url" placeholder="URL" required
        class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent" />

      <!-- Tag chips input -->
      <div class="relative">
        <div
          @click="focusInput"
          class="flex flex-wrap items-center gap-1.5 min-h-[38px] px-2 py-1.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 cursor-text focus-within:ring-2 focus-within:ring-primary-500 focus-within:border-transparent"
        >
          <span
          ...
          <input
            ...
            class="flex-1 min-w-[80px] bg-transparent border-none outline-none text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 p-0.5"
          />
        </div>
      ...
      <textarea v-model="notes" placeholder="Notes (optional)" rows="2"
        class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-y" />

      <div class="flex gap-2 pt-1">
        <button type="submit" :disabled="submitting"
          class="flex-1 py-2 rounded-lg bg-primary-600 text-white text-sm font-medium hover:bg-primary-700 disabled:opacity-50 transition">
